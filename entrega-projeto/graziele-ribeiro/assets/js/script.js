...
form.addEventListener('submit', async function getUsers(event) {
    ...
    if (result.status == 200) {
        ...
        let profileContent = `<section id="user-section">
        <div class="user-card">
            <img id="img-user" src="${data.avatar_url}" alt=""></img>
            <h3 id="name-user">${data.name ?? ''}</h3>
            <p id="nickname-user">${data.login}</p>
            <p id="bio-user">${data.bio ?? ''}</p>
            <div class="info-numbers">
                <div class="followers-card">
                    <img id="followers-img" src="/graziele-ribeiro/assets/images/people_outline.png" alt=""></img>
                    <p id="followers-user">${data.followers}</p>
                </div>
                <div class="followers-card">
                    <img id="repository-img" src="/graziele-ribeiro/assets/images/book_mark.png" alt=""></img>
                    <p id="repository-user">${data.public_repos}</p>
                </div>
            </div>
        </div>
    </section>`;
        ...
        if(data.public_repos > 0){
            ...
            dataRepositories.forEach((repository) => {
                reposContent += `<div class="card-repo">
                <div class="repo-content">
                    <p class="card-title">${repository.name ?? ''}</p>
                    <p class="card-body">${repository.description ?? ''}</p>
                    <div class = "repo-footer">
                    <div class="card-tech">
                        <div class="tech-circle" style="background-color:${languageColors[repository.language ?? 'JavaScript'].color}"></div>
                        <p class="tech-name">${repository.language ?? ''}</p>
                    </div>
                    <div class="card-stars">
                        <img src="/graziele-ribeiro/assets/images/star_border.png" alt="">
                        <p class="number-stars">${repository.stargazers_count}</p>
                    </div>
                    </div>
                    </div>
                </div>`;
            });
            reposContent = `<section id="public-repos"> ${reposContent} </section>`;
        }else{
            reposContent = `<div class="no-repo"> ${data.name ?? 'Este user'} não tem repositórios públicos ainda </div>`
        }

        mainDiv.innerHTML = profileContent + reposContent;
        body.style.height ="auto";


    } else {
        mainDiv.innerHTML = ` <div class="not-found-main">
     <p class="notFound-user"> Usuário não encontrado :(</p>
     <p class="search"> Pesquisa Novamente</p>
     <img src="./assets/images/notfound.png" alt="" class="not-found">
 </div>`;
    }
});
